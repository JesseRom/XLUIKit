...
#import "UIImage+XLCompress.h"
#import "UIImage+XLResize.h"

@implementation UIImage (XLCompress)

- (NSData *)xl_compressedJPEGImageData
{
    CGFloat compressRate = 0.3;
    CGSize imageSize     = [UIImage __xl_sizeForUploadWithImageSize:self.size compressRate:&compressRate];
    UIImage *targetImage = [self xl_resizedImage:imageSize interpolationQuality:kCGInterpolationDefault];
    
    return UIImageJPEGRepresentation(targetImage, compressRate);
}

+ (CGSize)__xl_sizeForUploadWithImageSize:(CGSize)size compressRate:(CGFloat *)compressRate
{
...
+ (UIImage *)xl_compressedImage:(UIImage *)image maxDataSize:(NSUInteger)dataSize {
    ...
    if (imageData.length > dataSize) {
        ...
        return [self __xl_compressedImage:image toScale:scaleSize maxDataSize:dataSize];
    } else {
        return image;
    }
}

+ (UIImage *)__xl_compressedImage:(UIImage *)image toScale:(float)scaleSize maxDataSize:(NSUInteger)dataSize {
    ...
    if (imageData.length > dataSize) {
        ...
        return [self __xl_compressedImage:scaledImage toScale:scale maxDataSize:dataSize];
    }
    return scaledImage;
}
